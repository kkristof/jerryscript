      ...
      char __attr_unused___ **argv)
{
  ...
  for (int i = 0; i < test_iters; i++)
  {
    ...
    {
      code_units[code_units_count] = lit_utf8_iterator_read_next (&iter);
    ...
    if (code_units_count > 0)
    {
      for (int j = 0; j < test_subiters; j++)
      {
        ...
        lit_utf8_iterator_restore_pos (&iter, saved_positions[index]);
        JERRY_ASSERT (lit_utf8_iterator_read_next (&iter) == code_units[index]);
        JERRY_ASSERT (lit_utf8_iterator_get_index (&iter) == index);
      }
    }

    lit_utf8_iterator_set_to_eos (&iter);
    ...
    while (!lit_utf8_iterator_is_bos (&iter))
    {
      JERRY_ASSERT (code_units_count > 0);
      calculated_length--;
      JERRY_ASSERT (code_units[calculated_length] == lit_utf8_iterator_read_prev_and_decr (&iter));
    }
